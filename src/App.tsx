...
export const App = () => {
  ...
  useEffect(() => {
    if (stocks?.question) {
      window.gtag('event', '7287_question_impression', { var: 'var5', question: stocks.id });
    }
  }, [stocks]);
  ...
  const handleAnswerClick = (option: PredictionOption) => {
    ...
    if (pressedOption) {
      window.gtag('event', '7287_answer_change', { var: 'var5', answer: option, question: stocks?.id ?? '' });
      setPressedOption(option);

      return;
    }

    window.gtag('event', '7287_answer_click', { var: 'var5', answer: option, question: stocks?.id ?? '' });
    setPressedOption(option);
  };
